    ...
    return (
      <div className={`min-h-screen bg-white ${className}`}>
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="bg-gradient-to-r from-tertiary/10 to-primarySupport/10 shadow-lg p-12 text-center">
                  ...
                  <div key={item.id} className="p-4 sm:p-6 hover:bg-gray-50 transition-colors">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                      ...
                      <div className="flex items-center justify-center sm:justify-end gap-3">
                        <button onClick={() => handleQuantityChange(item.productId, item.quantity - 1)} disabled={updatingItem === item.productId} className="w-10 h-10 flex items-center justify-center border-2 border-gray-300 hover:border-tertiary hover:bg-tertiary/10 disabled:opacity-50 transition-all"><Minus className="w-4 h-4" /></button>
                        <span className="w-16 text-center font-semibold text-lg">{updatingItem === item.productId ? <Loading size="sm" /> : item.quantity}</span>
                        <button onClick={() => handleQuantityChange(item.productId, item.quantity + 1)} disabled={updatingItem === item.productId} className="w-10 h-10 flex items-center justify-center border-2 border-gray-300 hover:border-tertiary hover:bg-tertiary/10 disabled:opacity-50 transition-all"><Plus className="w-4 h-4" /></button>
                      </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white shadow-lg border border-gray-100 p-6 sticky top-24">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="border-t border-gray-200 pt-4"><div className="flex justify-between items-center"><span className="text-xl font-bold text-gray-900">Total</span><span className="text-xl font-bold text-tertiary">{formatPrice(cart.total)}</span></div></div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <button onClick={clearCart} className="w-full text-gray-600 py-3 px-6 border-2 border-gray-300 hover:bg-gray-50 transition-colors font-medium">Clear Cart</button>
              </div>
